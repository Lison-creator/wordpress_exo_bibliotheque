...
class wallstreet_Customizer_Notify_Section extends WP_Customize_Section {
	
	public $type = 'wallstreet-customizer-notify-section';
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'wallstreet_cust_notify_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'wallstreet_cust_notify_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		...
		foreach ( $wallstreet_customizer_notify_recommended_actions as $key => $wallstreet_lite_customizer_notify_recommended_action ) {
			if ( $wallstreet_customizer_notify_show_recommended_actions[ $wallstreet_lite_customizer_notify_recommended_action['id'] ] === false ) {
				continue;
			}
			if ( $wallstreet_lite_customizer_notify_recommended_action['check'] ) {
				continue;
			}

			$wallstreet_lite_customizer_notify_recommended_action['index'] = $key + 1;

			if ( isset( $wallstreet_lite_customizer_notify_recommended_action['plugin_slug'] ) ) {
				$active = $this->check_active( $wallstreet_lite_customizer_notify_recommended_action['plugin_slug'] );
				$wallstreet_lite_customizer_notify_recommended_action['url'] = $this->create_action_link( $active['needs'], $wallstreet_lite_customizer_notify_recommended_action['plugin_slug'] );
				if ( $active['needs'] !== 'install' && $active['status'] ) {
					$wallstreet_lite_customizer_notify_recommended_action['class'] = 'active';
				} else {
					$wallstreet_lite_customizer_notify_recommended_action['class'] = '';
				}

				switch ( $active['needs'] ) {
					case 'install':
						$wallstreet_lite_customizer_notify_recommended_action['button_class'] = 'install-now button';
						$wallstreet_lite_customizer_notify_recommended_action['button_label'] = $install_button_label;
						break;
					case 'activate':
						$wallstreet_lite_customizer_notify_recommended_action['button_class'] = 'activate-now button button-primary';
						$wallstreet_lite_customizer_notify_recommended_action['button_label'] = $activate_button_label;
						break;
					case 'deactivate':
						$wallstreet_lite_customizer_notify_recommended_action['button_class'] = 'deactivate-now button';
						$wallstreet_lite_customizer_notify_recommended_action['button_label'] = $deactivate_button_label;
						break;
				}
			}
			$formatted_array[] = $wallstreet_lite_customizer_notify_recommended_action;
		}// End foreach().
		...
		foreach ( $wallstreet_customizer_notify_recommended_plugins as $slug => $plugin_opt ) {

			if ( ! $plugin_opt['recommended'] ) {
				continue;
			}

			if ( isset( $wallstreet_lite_customizer_notify_show_recommended_plugins[ $slug ] ) && $wallstreet_lite_customizer_notify_show_recommended_plugins[ $slug ] ) {
				continue;
			}
		...
		$json['total_actions']       = count( $wallstreet_customizer_notify_recommended_actions );
		...
		<# if( data.recommended_actions.length > 0 || data.recommended_plugins.length > 0 ){ #>
			<li id="accordion-section-{{ data.id }}" class="accordion-section control-section control-section-{{ data.type }} cannot-expand">
				...
				<h3 class="accordion-section-title">
					...
					<# if( data.recommended_actions.length > 0 ){ #>
						<span class="wallstreet-customizer-plugin-notify-actions-count">
